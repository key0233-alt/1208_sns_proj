...
- 환경별 Supabase 클라이언트 분리:
  ...
  - Server Component/Server Action용 (`createServerSupabaseClient`)
...
자세한 설정 방법은 [`docs/CLERK_SUPABASE_SETUP.md`](./docs/CLERK_SUPABASE_SETUP.md)를 참고하세요.
...
2. **Supabase Dashboard**:
   ...
   - **"Add Provider"** 클릭
   - Provider Name: `Clerk`
   - JWT Issuer: `https://your-app-12.clerk.accounts.dev`
   - JWKS URI: `https://your-app-12.clerk.accounts.dev/.well-known/jwks.json`
   - **"Save"** 클릭

3. **통합 확인**:
   - `/auth-test` 페이지에서 테스트
   - Supabase Dashboard → Authentication → Users에서 Clerk 사용자 확인
...
또는 `supabase/migrations/example_rls_policies.sql` 파일을 참고하여 다른 예제 테이블을 생성할 수 있습니다.
...
**커스터마이징:**
특정 텍스트나 에러 메시지를 커스터마이징하려면 `app/layout.tsx`에서 `koreanLocalization` 객체를 수정하세요.
...
│   │   ├── server.ts          # Server Component/Action용 (createServerSupabaseClient)
│   │   ├── service-role.ts    # 관리자용 (getServiceRoleClient)
│   │   └── client.ts          # 공개 데이터용
...
- **`lib/supabase/`**: 환경별 Supabase 클라이언트 (매우 중요!)
  - `clerk-client.ts`: Client Component용 (`useClerkSupabaseClient`)
  - `server.ts`: Server Component/Action용 (`createServerSupabaseClient`)
  - `service-role.ts`: 관리자 권한용 (`getServiceRoleClient`)
- **`hooks/use-sync-user.ts`**: Clerk 사용자를 Supabase에 자동 동기화
- **`components/providers/sync-user-provider.tsx`**: 앱 전역에서 사용자 동기화 실행
- **`docs/CLERK_SUPABASE_SETUP.md`**: Clerk + Supabase 통합 상세 설정 가이드
- **`docs/CLERK_LOCALIZATION.md`**: Clerk 한국어 로컬라이제이션 가이드
